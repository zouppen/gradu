...
käyttäen luotetun koneen IP:tä (kuva \ref{IP-spoofing}). Ensimmäisessä
vaiheessa hyökkääjän tulee selvittää luotetun kohteen IP:een käyttäen joko sokeaa 
...
toiminto on taas sallittu, koska toimiakseen jotkut IP-pohjaiset palvelut joutuvat
...
Tämä tieto taas tarvitaan, koska ilman sitä
verkko ei pysty välittämään paketteja oikealle koneelle. ARP-spoofing, jota
kutsutaan myös ARP-myrkyttämiseksi, pyrkii hyödyntämään ARP-protokollan
...
Tämän jälkeen kohdekoneelle tarkoitetut paketit ohjautuvat hyökkääjän koneelle[7].
...
vastata ACK-viestillä, jonka jälkeen yhteys muodostetaan [2].
...
määritetty raja yhteyden aikakatkaisulle tulee vastaan [6]. Hyökkääjälle
...
ylittyessä aletaan yhteyksiä pudottaa [5]. Toinen käytetty keino on
...
liikenteen seuraamiseen tarkoitetuilla sovelluksilla sekä pääsylistoilla [6].
...
on vain sallittu verkossa. Näistä hyökkäyksistä tunnetuin lienee Fraggleksi
nimetty hyökkäys, joka nykyisinkin  aiheuttaa väärin konfiguroidussa verkossa suuria ongelmia.
Fraggle toimii siten, että hyökkääjä lähettää UDP
...
tuottaa 25 Kbps liikennettä, aiheuttaa yhteensä 75 Mbps kuorman verkolle [2].
...
organisaation toiminnan useiksi päiviksi [1].
...
suurin koneista pitää tunnistaa ja liikennöinti pysäyttää, jotta tästä olisi
...
liikenteestä, koska se tulee palvelimelle eri reittejä pitkin [11].
...
haltuunoton vaatimatta sen suurempaa tietämystä [11].
...
annetut käskyt agenteille hyökkäyksen alettua [7] [11]. Ensimmäisissä DDoS-
...
poistamaan verkosta, jolloin hyökkäyksessä käytetty verkko hajosi [11].
...
ja reagoiminen, ja ne noudattavat yleistä vahinkoketjun elinkaarta [11].
...
tarvittavat toimenpiteet hyökkäyksen lopettamiseksi. Useimmissa tapauksissa
...
muita hyökkäyksiä huomattavasti suuremmat [9]. Onnistunut R2L-hyökkäys saattaa
...
löydetty näille alustoille [8]. Useat haavoittuvaisuudet johtuvat
...
haavoittuvuuteen on olemassa ennen kuin sitä on mahdollista hyödyntää [9].
...
riittävän tarkasti erottamaan haitallista liikennettä normaalista [16].
